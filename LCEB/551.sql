...
WHERE akat.title LIKE 'K_w_ _u_s for Hire'
  AND t.title LIKE 'River%ian_'
  AND k.keyword LIKE 's_per-sonic'
  AND n.name LIKE 'Moore%R_en'
  AND cn.name LIKE '_y_i%ad%'
  AND c.name LIKE '%pe_n%n_F_l%oduk%o_'
