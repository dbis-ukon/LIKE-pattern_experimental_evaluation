...
WHERE akat.title LIKE '_e% _est_V_l% _'
  AND t.title LIKE '%I_n% Man'
  AND k.keyword LIKE 'k%t%g%he_d'
  AND c.name LIKE 'Image Box (I_)'
  AND n.name LIKE 'G_rvin'
  AND cn.name LIKE 'Hims_l_ - Corr_spondent Ira_'
  AND ci.nr_order <= 29
