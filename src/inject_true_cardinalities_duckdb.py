...
def sql_escape(pattern: str) -> str:
    return pattern.replace("'", "''")


# -----------------------
# EXACT SELECTIVITY, COMPUTED LIVE FROM THE DATABASE
# -----------------------

def extract_number(cli_output: str) -> str:
    for line in cli_output.splitlines():
        inner = line.strip().strip("│").strip()
        if inner and inner.replace(",", "").isdigit():
            return inner
    return "?"


class DatabaseSelectivityComputer:
    def __init__(self, duckdb_binary: str, database: str):
        self.duckdb_binary = duckdb_binary
        self.database = database
        self._total_count_cache = {}
        self._selectivity_cache = {}
        self.db_queries_run = 0
        self.cache_hits = 0

    def _run_count(self, sql: str):
        self.db_queries_run += 1
        result = subprocess.run(
            [self.duckdb_binary, self.database, "-c", sql],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            return None
        count_str = extract_number(result.stdout)
        if count_str == "?":
            return None
        return int(count_str.replace(",", ""))

    def _get_total(self, table: str):
        if table in self._total_count_cache:
            return self._total_count_cache[table]
        total = self._run_count(f'SELECT count(*) FROM "{table}";')
        self._total_count_cache[table] = total
        return total

    def get_selectivity(self, table: str, column: str, pattern: str):
        ...
        if not total:
            self._selectivity_cache[key] = None
            return None

        escaped_pattern = sql_escape(pattern)
        matched = self._run_count(
            f"SELECT count(*) FROM \"{table}\" WHERE \"{column}\" LIKE '{escaped_pattern}';"
        )
        if matched is None:
            self._selectivity_cache[key] = None
            return None
    ...
    def summary(self):
        print(f"\n  Selectivity computation summary: {self.db_queries_run} DuckDB queries run, "
              f"{self.cache_hits} cache hits (patterns reused across queries), "
              f"{len(self._total_count_cache)} distinct table(s) had their total row count cached.")
...
def extract_injectable_patterns(stmt: str, computer: DatabaseSelectivityComputer) -> dict:
    ...
    for alias, column, op, raw_pattern in _PRED_LIKE_RE.findall(stmt):
        ...
        selectivity = computer.get_selectivity(table, column, pattern)
        if selectivity is not None:
            matched[pattern] = selectivity
...
def log_injected_patterns(log_path: str, query_no: int, matched: dict):
    with open(log_path, "a", encoding="utf-8") as f:
        ...
        if matched:
            ...
            f.write("(no patterns injected - selectivity could not be computed)\n")
        f.write("\n")
...
def main():
    ...
    computer = DatabaseSelectivityComputer(DUCKDB_BINARY, DATABASE)
    ...
    for i, stmt in enumerate(statements):
        ...
        matched = extract_injectable_patterns(stmt, computer)
        write_like_selectivity_file(matched)
        log_injected_patterns(INJECTION_LOG_FILE, i + 1, matched)
        if matched:
            print(f"    injecting {len(matched)} pattern(s) for this query (exact, computed from DB)")
